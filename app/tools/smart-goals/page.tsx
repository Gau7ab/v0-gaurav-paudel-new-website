...
export default function SMARTGoalsPage() {
  ...
  const generateActionItems = () => {
    ...
    if (data.specific) {
      items.push("Define clear success criteria based on your specific goal")
    }
    if (data.measurable) {
      items.push("Set up tracking system for your metrics")
    }
    if (data.achievable) {
      items.push("Identify and secure necessary resources")
    }
    if (data.relevant) {
      items.push("Communicate goal alignment with stakeholders")
    }
    if (data.timeBound) {
  ...
  if (showResults) {
    ...
    return (
      <ToolLayout
        ...
        onReset={handleReset}
      >
        <div id="smart-results" className="space-y-8">
          {/* Before & After */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="border-primary/30 bg-primary/5">
              ...
              <CardContent>
                <p className="font-medium">{smartStatement}</p>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent>
              <div className="space-y-4">
                {steps.slice(1).map((step) => (
                  <div key={step.key} className="flex gap-4 py-3 border-b border-border last:border-0">
                    ...
                    {data[step.key] && <CheckCircle className="h-5 w-5 text-chart-2 shrink-0" />}
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Goal Quality Score</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center gap-4">
                <div className="text-3xl font-bold text-primary">
                  {Object.values(data).filter((v) => v.trim()).length * 17}%
                </div>
                <div className="flex-1">
                  <div className="w-full bg-secondary rounded-full h-3">
                    <div
                      ...
                      style={{
                        width: `${Object.values(data).filter((v) => v.trim()).length * 17}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              <p className="text-sm text-muted-foreground mt-3">
                {Object.values(data).filter((v) => v.trim()).length === 6
    ...
    >
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="bg-muted/50 rounded-lg p-3 text-sm text-muted-foreground">{currentStepData.hint}</div>

          <div className="space-y-2">
            <Label htmlFor="step-input">Your response</Label>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={nextStep} className="gap-2">
          {currentStep === 6 ? "Create Goal" : "Next"}
